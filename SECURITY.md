...
Run the release checks in [Development](docs/DEVELOPMENT.md) before distributing a modified build. The protected workflow design, required secrets, approval boundary, severity gates, and digest-based verification commands are in [Container release and verification](docs/CONTAINER_RELEASE.md). Image signing, SBOM/provenance publication, vulnerability-scan results, and full macOS/Linux runtime evidence remain release work until the exact artifacts and results are published; do not infer those assurances from the source tree alone.
