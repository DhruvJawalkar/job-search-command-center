...
type SummaryScheduleBlock = { id: string; dayPart: SummaryDayPart; startTime: string; endTime: string;
  source: SummaryScheduleSource; customTitle: string | null; customDetail: string | null };
type SummaryPriorityPreference = { source: SummaryPrioritySource; label: string; timeLabel: string;
  customTitle: string | null; customDetail: string | null };
type SummaryPreferences = { scheduleBlocks: SummaryScheduleBlock[]; priorities: SummaryPriorityPreference[];
  specializationMode: "ROTATING" | "FIXED"; fixedSpecializationDay: number | null;
  recommendationFocus: "BALANCED" | "OPPORTUNITIES" | "NETWORK" | "PREPARATION";
  weeklyApplicationTarget: number;
  showDailyPerspective: boolean; showMarketLens: boolean; showTechnologyWatch: boolean;
...
const defaultSummaryPreferences: SummaryPreferences = {
  scheduleBlocks: [
    ...
    { id: "breakfast", dayPart: "MORNING", startTime: "07:30", endTime: "08:30", source: "BREAK", customTitle: "Breakfast and reset", customDetail: null },
    { id: "opportunity-primary", dayPart: "MORNING", startTime: "08:30", endTime: "10:50", source: "OPPORTUNITY", customTitle: null, customDetail: null },
    { id: "outreach-morning", dayPart: "MORNING", startTime: "11:00", endTime: "11:30", source: "OUTREACH", customTitle: null, customDetail: null },
    { id: "lunch", dayPart: "MORNING", startTime: "11:30", endTime: "12:30", source: "BREAK", customTitle: "Lunch break", customDetail: null },
    { id: "specialization-primary", dayPart: "AFTERNOON", startTime: "12:30", endTime: "14:25", source: "SPECIALIZATION", customTitle: null, customDetail: null },
    { id: "opportunity-secondary", dayPart: "AFTERNOON", startTime: "14:30", endTime: "15:00", source: "OPPORTUNITY", customTitle: null, customDetail: null },
    { id: "short-break", dayPart: "AFTERNOON", startTime: "15:00", endTime: "15:15", source: "BREAK", customTitle: "Short break", customDetail: null },
    { id: "outreach-afternoon", dayPart: "AFTERNOON", startTime: "15:15", endTime: "15:45", source: "OUTREACH", customTitle: null, customDetail: null },
    { id: "specialization-secondary", dayPart: "AFTERNOON", startTime: "15:45", endTime: "17:00", source: "SPECIALIZATION", customTitle: null, customDetail: null },
    { id: "end-of-day", dayPart: "EVENING", startTime: "18:30", endTime: "19:30", source: "END_OF_DAY", customTitle: null, customDetail: null },
  ],
  priorities: [
    { source: "CODING", label: "Daily anchor", timeLabel: "5:00–7:30 AM", customTitle: null, customDetail: null },
    { source: "OPPORTUNITY", label: "Daily anchor", timeLabel: "8:30–10:50 AM", customTitle: null, customDetail: null },
    { source: "SPECIALIZATION", label: "Specialization", timeLabel: "12:30–2:25 PM", customTitle: null, customDetail: null },
  ],
  specializationMode: "ROTATING", fixedSpecializationDay: 1, recommendationFocus: "BALANCED", weeklyApplicationTarget: 10,
  showDailyPerspective: true, showMarketLens: true, showTechnologyWatch: true, updatedAt: null, version: 0,
};
...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const [nextDashboard, nextResumes, nextApplications, nextTrackedOpportunities, nextFeed, nextArchivedFeed, nextActions, nextImports, nextContacts, nextOutreach, nextPreparation, nextCandidates, nextSkills, nextBacklog, nextLinkedIn, nextInbox, nextWeeklyReviews, nextMonthlyProgress, nextCalendarEvents, nextCalendarReminders, nextProfile, nextInstallation, nextSummaryPreferences] = await Promise.all([
        ...
        api<SummaryPreferences>("/api/v1/summary-preferences"),
      ]);
  ...
  const weeklyApplicationTarget = summaryPreferences.weeklyApplicationTarget || 10;
  const weeklyApplicationProgress = Math.min(100, Math.round((dashboard.appliedThisWeek / weeklyApplicationTarget) * 100));
  const weeklyApplicationsRemaining = Math.max(0, weeklyApplicationTarget - dashboard.appliedThisWeek);
  ...
  const todayFocusPlan = dailyFocusPlanForDate(dashboard.date, summaryPreferences);
  ...
  const nextOutreachAction = [...outreach].filter((item) => !["CLOSED", "DECLINED", "REFERRED"].includes(item.status))
    .sort((left, right) => Number(right.overdue) - Number(left.overdue) || (left.followUpAt ?? "9999").localeCompare(right.followUpAt ?? "9999"))[0] ?? null;
  const nextApplicationFollowUp = [...dashboard.actions].sort((left, right) => left.nextActionAt.localeCompare(right.nextActionAt))[0] ?? null;
  const topDailyPriorities = summaryPreferences.priorities.map((preference, index) => {
    const common = { index: String(index + 1).padStart(2, "0"), label: preference.label, time: preference.timeLabel };
    switch (preference.source) {
      case "CODING": return { ...common, title: "Coding practice", detail: nextCodingTask
        ? `${nextCodingTask.item.title} · ${nextCodingTask.track.name}` : "Timed problem solving, implementation, edge cases, and complexity review." };
      case "OPPORTUNITY": return { ...common, title: "Opportunity execution", detail: nextOpportunityAction?.actionText
        ?? "Review and progress the strongest application-ready opening." };
      case "OUTREACH": return { ...common, title: nextOutreachAction ? `${nextOutreachAction.overdue ? "Follow up with" : "Progress outreach to"} ${nextOutreachAction.contactName}` : "Referral and outreach",
        detail: nextOutreachAction ? `${nextOutreachAction.companyName} · ${nextOutreachAction.roleTitle}` : "Progress the strongest trusted path for a high-fit opening." };
      case "APPLICATION_FOLLOW_UP": return { ...common, title: nextApplicationFollowUp?.nextAction ?? "Application follow-ups",
        detail: nextApplicationFollowUp ? `${nextApplicationFollowUp.companyName} · ${nextApplicationFollowUp.roleTitle}` : "Review active applications and set the next action." };
      case "CUSTOM": return { ...common, title: preference.customTitle || "Custom priority", detail: preference.customDetail || "Define the outcome that will make this priority complete." };
      default: return { ...common, label: preference.label || todayFocusPlan.label, title: todayFocusPlan.title,
  ...
  const balancedRecommendation = dueOutreach > 0
    ? `Close the loop on ${dueOutreach} overdue outreach follow-up${dueOutreach === 1 ? "" : "s"} before starting new networking, then return to the highest-fit opportunity.`
    : dashboard.actions.length > 0
      ...
      : nextOpportunityAction ? `Complete today’s highest-ranked opening action before expanding the shortlist: ${nextOpportunityAction.actionText}`
        : `Make one measurable ${todayFocusPlan.label.toLowerCase()} deliverable the definition of done for today.`;
  const coachRecommendation = summaryPreferences.recommendationFocus === "OPPORTUNITIES"
    ? nextOpportunityAction ? `Start with the highest-ranked opening action: ${nextOpportunityAction.actionText}`
      : nextApplicationFollowUp ? `Protect pipeline momentum with ${nextApplicationFollowUp.companyName}: ${nextApplicationFollowUp.nextAction}`
        : "Review the strongest active opening and record one explicit pursue, referral, apply, or skip decision."
    : summaryPreferences.recommendationFocus === "NETWORK"
      ? nextOutreachAction ? `${nextOutreachAction.overdue ? "Close the overdue loop" : "Progress the relationship"} with ${nextOutreachAction.contactName} for ${nextOutreachAction.companyName}.`
        : "Choose one high-fit opening and identify the strongest credible referral path before broadening outreach."
      : summaryPreferences.recommendationFocus === "PREPARATION"
        ? nextFocusTask ? `Move today’s ${todayFocusPlan.label.toLowerCase()} work forward: ${nextFocusTask.item.title} in ${nextFocusTask.track.name}.`
          : `Create one measurable ${todayFocusPlan.label.toLowerCase()} practice outcome and preserve evidence when it is complete.`
  ...
  async function reviewSelectedSkillObservations(status: Exclude<SkillReviewStatus, "PROPOSED">) {
    ...
    try {
      await api("/api/v1/skills/observations/review", { method: "PATCH", body: JSON.stringify({
        ...
        note: status === "ACCEPTED" ? "Validated in the human review queue." : "Excluded during human review.",
      }) });
    ...
    <main className="app-shell">
      <header className="top-navigation">
        ...
        <div className="topbar-actions">
          ...
          {activeWorkspace === "overview" && <button className="secondary-button summary-customize-button" type="button" onClick={() => setSummaryConfigurationOpen(true)}>Customize Summary</button>}
        ...
        <div className="content-wrap">
          ...
          <section className="hero">
            <div className="hero-greeting"><p className="eyebrow">Morning command center</p><h1>Good morning, {profile.displayName || "there"}.</h1>
              {summaryPreferences.showDailyPerspective && <div className="daily-motivation"><span>Daily perspective</span><blockquote>“{dailyMotivation}”</blockquote></div>}</div>
            <aside className="daily-briefing" aria-label="Personalized daily recommendations">
              <header><div><span>Your personalized briefing</span><strong>Recommended next move</strong></div><em>Based on your workspace</em></header>
              <p>{coachRecommendation}</p>
              {(summaryPreferences.showMarketLens || summaryPreferences.showTechnologyWatch) && <div className="briefing-signals">
                {summaryPreferences.showMarketLens && <article><span>Market lens</span><strong>{topMarketSignal
                ...
                {summaryPreferences.showTechnologyWatch && <article><span>{technologyWatch.theme}</span><strong>{technologyWatch.title}</strong></article>}
              </div>}
              {summaryPreferences.showTechnologyWatch && <div className="technology-watch"><p>{technologyWatch.insight}</p><small>{technologyWatch.prompt}</small>
          ...
          <section className="overview-operating-row" aria-label="Daily priorities, weekly applications, and pipeline health">
            <section className="daily-workspace">
              <article className="panel daily-plan-panel">
                <div className="daily-plan-context"><p>Your selected priority sources stay grounded in live opportunity, relationship, and preparation data.</p>
            ...
            <section className="weekly-application-goal" aria-label={`${dashboard.appliedThisWeek} of ${weeklyApplicationTarget} weekly applications completed`}>
              <header><div><p className="eyebrow">Weekly application goal</p><strong>{dashboard.appliedThisWeek} of {weeklyApplicationTarget} applications</strong></div>
                <span>{weeklyApplicationProgress}% complete</span></header>
              <div className="weekly-goal-track" role="progressbar" aria-valuemin={0} aria-valuemax={weeklyApplicationTarget}
                aria-valuenow={Math.min(dashboard.appliedThisWeek, weeklyApplicationTarget)}><i style={{ width: `${weeklyApplicationProgress}%` }} /></div>
              <footer><span>{weeklyApplicationsRemaining > 0 ? `${weeklyApplicationsRemaining} applications remaining this week` : "Weekly target reached—excellent momentum."}</span>
                <small>Target · {weeklyApplicationTarget}</small></footer>
            </section>
                ...
                <div className="application-artifact-list">{applications.map((application) => {
                  ...
                  return <article key={application.id}><header><div><strong>{application.companyName}</strong><span>{application.roleTitle}</span></div><em>{stageLabels[application.stage]}</em></header>
                    ...
                    {resumeArtifact ? <div className="artifact-proof"><span>PDF</span><div><strong>{resumeArtifact.storedPath}</strong><small>SHA-256 {resumeArtifact.contentHash.slice(0, 12)}… · {formatFileSize(resumeArtifact.sizeBytes)}</small></div></div>
                      : <p className="artifact-missing">No immutable resume file is preserved for this application.</p>}
          ...
          <TodaySchedule date={dashboard.date} dailyActions={dailyActions.actions} preparation={preparation}
            applicationFollowUps={dashboard.actions} outreach={outreach} focusPlan={todayFocusPlan} preferences={summaryPreferences} />
          </div>}
          ...
          {activeWorkspace === "overview" && <section className="roadmap-strip" id="roadmap">
            <div><p className="eyebrow">Focused workspaces</p><h2>One command center, seven deliberate views, and less cognitive overhead.</h2></div>
            <div className="roadmap-items"><span><b>LIVE</b> Summary</span><span><b>LIVE</b> Opportunity intake</span>
      ...
      {opportunityModal && <OpportunityModal connected={connected} onClose={() => setOpportunityModal(false)}
        ...
        onError={(message) => setToast({ kind: "error", message })} />}
      {summaryConfigurationOpen && <SummaryPreferencesModal preferences={summaryPreferences} connected={connected}
        onClose={() => setSummaryConfigurationOpen(false)} onSaved={(saved) => {
...
const summaryPrioritySourceLabels: Record<SummaryPrioritySource, string> = {
  CODING: "Coding practice", OPPORTUNITY: "High-fit opportunity", OUTREACH: "Network outreach",
  SPECIALIZATION: "Today’s specialization", APPLICATION_FOLLOW_UP: "Application follow-up", CUSTOM: "Custom priority",
};
const summaryScheduleSourceLabels: Record<SummaryScheduleSource, string> = {
  CODING: "Coding practice", OPPORTUNITY: "High-fit opportunity", OUTREACH: "Network outreach",
  SPECIALIZATION: "Today’s specialization", APPLICATION_FOLLOW_UP: "Application follow-up", BREAK: "Protected break",
  CUSTOM: "Custom focus block", END_OF_DAY: "Best remaining action",
};
const summaryDayPartLabels: Record<SummaryDayPart, string> = { MORNING: "Morning", AFTERNOON: "Afternoon", EVENING: "Evening" };

function cloneSummaryPreferences(preferences: SummaryPreferences): SummaryPreferences {
  return { ...preferences, scheduleBlocks: preferences.scheduleBlocks.map((block) => ({ ...block })),
    priorities: preferences.priorities.map((priority) => ({ ...priority })) };
}

function SummaryPreferencesModal({ preferences, connected, onClose, onSaved }: {
  preferences: SummaryPreferences; connected: boolean; onClose: () => void; onSaved: (saved: SummaryPreferences) => void;
}) {
  const [draft, setDraft] = useState(() => cloneSummaryPreferences(preferences));
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const prioritySources = Object.keys(summaryPrioritySourceLabels) as SummaryPrioritySource[];
  const scheduleSources = Object.keys(summaryScheduleSourceLabels) as SummaryScheduleSource[];
  const dayParts = Object.keys(summaryDayPartLabels) as SummaryDayPart[];

  function updatePriority(index: number, changes: Partial<SummaryPriorityPreference>) {
    setDraft((current) => ({ ...current, priorities: current.priorities.map((priority, currentIndex) =>
      currentIndex === index ? { ...priority, ...changes } : priority) }));
  }
  function updateSchedule(id: string, changes: Partial<SummaryScheduleBlock>) {
    setDraft((current) => ({ ...current, scheduleBlocks: current.scheduleBlocks.map((block) =>
      block.id === id ? { ...block, ...changes } : block) }));
  }
  function addScheduleBlock() {
    if (draft.scheduleBlocks.length >= 12) return;
    const id = `custom-${Date.now()}`;
    setDraft((current) => ({ ...current, scheduleBlocks: [...current.scheduleBlocks,
      { id, dayPart: "EVENING", startTime: "17:00", endTime: "18:00", source: "CUSTOM",
        customTitle: "Personal focus block", customDetail: "Describe the outcome you want to complete." }] }));
  }
  function removeScheduleBlock(id: string) {
    if (draft.scheduleBlocks.length <= 1) return;
    setDraft((current) => ({ ...current, scheduleBlocks: current.scheduleBlocks.filter((block) => block.id !== id) }));
  }
  async function save(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setSaving(true); setError(null);
    try {
      const saved = await api<SummaryPreferences>("/api/v1/summary-preferences", { method: "PUT", body: JSON.stringify({
        scheduleBlocks: draft.scheduleBlocks, priorities: draft.priorities,
        specializationMode: draft.specializationMode,
        fixedSpecializationDay: draft.specializationMode === "FIXED" ? draft.fixedSpecializationDay : null,
        recommendationFocus: draft.recommendationFocus, weeklyApplicationTarget: draft.weeklyApplicationTarget,
        showDailyPerspective: draft.showDailyPerspective,
        showMarketLens: draft.showMarketLens, showTechnologyWatch: draft.showTechnologyWatch,
      }) });
      onSaved(saved);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not save the Summary configuration."); }
    finally { setSaving(false); }
  }

  return <Modal title="Customize Summary" subtitle="Shape the daily plan, top priorities, and personalized guidance shown on your local Summary." onClose={onClose} wide>
    <form className="summary-config-form" onSubmit={save}>
      <section className="summary-config-section"><header><div><span>01</span><h3>Recommendation and perspective</h3></div><p>Choose what the briefing should emphasize and which supporting insight cards belong on your Summary.</p></header>
        <div className="summary-config-grid"><Field label="Recommendation focus"><select value={draft.recommendationFocus}
          onChange={(event) => setDraft((current) => ({ ...current, recommendationFocus: event.target.value as SummaryPreferences["recommendationFocus"] }))}>
          <option value="BALANCED">Balanced next-best action</option><option value="OPPORTUNITIES">High-fit opportunities</option>
          <option value="NETWORK">Network and referrals</option><option value="PREPARATION">Preparation and skill growth</option></select></Field>
          <div className="summary-display-options" aria-label="Summary insight visibility">
            {([ ["showDailyPerspective", "Daily perspective"], ["showMarketLens", "Market lens"], ["showTechnologyWatch", "Technology watch"] ] as const).map(([key, label]) =>
              <label className="summary-toggle" key={key}><input type="checkbox" aria-label={label} checked={draft[key]}
                onChange={(event) => setDraft((current) => ({ ...current, [key]: event.target.checked }))} /><span><b>{label}</b><small>Show on Summary</small></span></label>)}</div></div>
      </section>

      <section className="summary-config-section"><header><div><span>02</span><h3>Today’s specialization</h3></div><p>Rotate the specialization by weekday or keep one focus active until you choose to change it.</p></header>
        <div className="summary-config-grid"><Field label="Specialization behavior"><select value={draft.specializationMode}
          onChange={(event) => setDraft((current) => ({ ...current, specializationMode: event.target.value as SummaryPreferences["specializationMode"] }))}>
          <option value="ROTATING">Rotate by weekday</option><option value="FIXED">Use one fixed specialization</option></select></Field>
          <Field label="Fixed specialization"><select value={draft.fixedSpecializationDay ?? 1} disabled={draft.specializationMode !== "FIXED"}
            onChange={(event) => setDraft((current) => ({ ...current, fixedSpecializationDay: Number(event.target.value) }))}>
            {Object.entries(weeklyFocusPlans).map(([day, plan]) => <option value={day} key={day}>{plan.label}</option>)}</select></Field></div>
      </section>

      <section className="summary-config-section"><header><div><span>03</span><h3>Weekly application goal</h3></div><p>Start with a manageable quality-focused target, then adjust it after seeing your first week of imported openings.</p></header>
        <div className="summary-weekly-goal-config"><Field label="Applications per week"><input type="number" required min={1} max={100}
          value={draft.weeklyApplicationTarget} onChange={(event) => setDraft((current) => ({ ...current, weeklyApplicationTarget: Number(event.target.value) }))} /></Field>
          <p><strong>Recommended default · 10</strong><span>This is optional setup—the default works immediately, and you can revisit it after your first import.</span></p></div>
      </section>

      <section className="summary-config-section"><header><div><span>04</span><h3>Top three priorities</h3></div><p>Each position draws from live workspace data. The label and time window remain yours to define.</p></header>
        <div className="summary-priority-list">{draft.priorities.map((priority, index) => <article className="summary-priority-editor" key={index}>
          <strong>Priority {index + 1}</strong><div className="summary-config-grid three"><Field label="Source"><select value={priority.source}
            onChange={(event) => updatePriority(index, { source: event.target.value as SummaryPrioritySource })}>
            {prioritySources.map((source) => <option value={source} key={source}>{summaryPrioritySourceLabels[source]}</option>)}</select></Field>
            <Field label="Label"><input required maxLength={80} value={priority.label} onChange={(event) => updatePriority(index, { label: event.target.value })} /></Field>
            <Field label="Time or cadence"><input required maxLength={80} value={priority.timeLabel} onChange={(event) => updatePriority(index, { timeLabel: event.target.value })} /></Field></div>
          {priority.source === "CUSTOM" && <div className="summary-config-grid"><Field label="Custom priority title"><input required maxLength={160} value={priority.customTitle ?? ""}
            onChange={(event) => updatePriority(index, { customTitle: event.target.value })} /></Field><Field label="Supporting detail"><input maxLength={500} value={priority.customDetail ?? ""}
            onChange={(event) => updatePriority(index, { customDetail: event.target.value })} /></Field></div>}
        </article>)}</div>
      </section>

      <section className="summary-config-section"><header><div><span>05</span><h3>Daily schedule</h3></div><p>Set one to twelve non-overlapping blocks. Dynamic sources use the most relevant live item available when the Summary loads.</p></header>
        <div className="summary-schedule-list">{draft.scheduleBlocks.map((block, index) => <article className="summary-schedule-editor" key={block.id}>
          <header><strong>Block {index + 1}</strong><button type="button" className="text-button destructive" disabled={draft.scheduleBlocks.length <= 1} onClick={() => removeScheduleBlock(block.id)}>Remove</button></header>
          <div className="summary-schedule-fields"><Field label="Daypart"><select value={block.dayPart} onChange={(event) => updateSchedule(block.id, { dayPart: event.target.value as SummaryDayPart })}>
            {dayParts.map((part) => <option value={part} key={part}>{summaryDayPartLabels[part]}</option>)}</select></Field>
            <Field label="Starts"><input type="time" required value={block.startTime} onChange={(event) => updateSchedule(block.id, { startTime: event.target.value })} /></Field>
            <Field label="Ends"><input type="time" required value={block.endTime} onChange={(event) => updateSchedule(block.id, { endTime: event.target.value })} /></Field>
            <Field label="Content source"><select value={block.source} onChange={(event) => updateSchedule(block.id, { source: event.target.value as SummaryScheduleSource })}>
              {scheduleSources.map((source) => <option value={source} key={source}>{summaryScheduleSourceLabels[source]}</option>)}</select></Field></div>
          {(block.source === "CUSTOM" || block.source === "BREAK") && <div className="summary-config-grid"><Field label={block.source === "BREAK" ? "Break label" : "Custom block title"}>
            <input required={block.source === "CUSTOM"} maxLength={160} value={block.customTitle ?? ""} onChange={(event) => updateSchedule(block.id, { customTitle: event.target.value })} /></Field>
            <Field label="Supporting detail"><input maxLength={500} value={block.customDetail ?? ""} onChange={(event) => updateSchedule(block.id, { customDetail: event.target.value })} /></Field></div>}
        </article>)}</div>
        <button type="button" className="secondary-button summary-add-block" disabled={draft.scheduleBlocks.length >= 12} onClick={addScheduleBlock}>+ Add schedule block</button>
      </section>
      {error && <p className="form-error" role="alert">{error}</p>}
      <div className="summary-config-actions"><button type="button" className="text-button" onClick={() => { setDraft(cloneSummaryPreferences(defaultSummaryPreferences)); setError(null); }}>Reset to recommended defaults</button>
        <div><button type="button" className="text-button" onClick={onClose}>Cancel</button><button className="primary-button" disabled={!connected || saving}>{saving ? "Saving…" : "Save Summary"}</button></div></div>
    </form>
  </Modal>;
}

function ProfileSettingsForm({ profile, connected, onSaved }: {
  profile: LocalProfile; connected: boolean; onSaved: (profile: LocalProfile) => void;
}) {
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  async function save(event: FormEvent<HTMLFormElement>) {
...
function Scratchpad({ connected, onSaved, onError }: { connected: boolean; onSaved: (message: string) => void; onError: (message: string) => void }) {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      try { setNote(window.localStorage.getItem(storageKey) ?? ""); } catch { /* Browser storage can be unavailable in private contexts. */ }
    }, 0);
    return () => window.clearTimeout(timer);
  }, []);
  useEffect(() => {
...
function dailyFocusPlanForDate(value: string, preferences: SummaryPreferences) {
  const rotatingDay = new Date(`${value}T08:00:00`).getDay();
  const day = preferences.specializationMode === "FIXED" ? preferences.fixedSpecializationDay ?? rotatingDay : rotatingDay;
...
function TodaySchedule({ date, dailyActions, preparation, applicationFollowUps, outreach, focusPlan, preferences }: {
  date: string; dailyActions: DailyAction[]; preparation: PreparationOverview; applicationFollowUps: ActionItem[]; outreach: Outreach[];
  focusPlan: DailyFocusPlan; preferences: SummaryPreferences;
}) {
  ...
  const breakBlock = (title: string, detail?: string | null): TodayScheduleEntry => ({ time: "", end: "", title,
    objective: "Recovery window", context: detail || "Step away fully, reset, and protect the quality of the next focus block.", status: "Protected", tone: "recovery" });
  const endOfDay = (): TodayScheduleEntry => outreachQueue[outreachIndex] ? referral() : actionQueue[actionIndex]
  ...
  const resolveBlock = (block: SummaryScheduleBlock): TodayScheduleEntry => {
    const entry = block.source === "CODING" ? prep("Coding practice", /(coding|problem|algorithm|thread|concurren|memory|production engineering)/i)
      : block.source === "OPPORTUNITY" ? action("Progress the strongest application-ready opening")
      : block.source === "OUTREACH" ? referral()
      : block.source === "SPECIALIZATION" ? prep(focusPlan.title, focusPlan.pattern, false, focusPlan.description)
      : block.source === "APPLICATION_FOLLOW_UP" ? followUp()
      : block.source === "BREAK" ? breakBlock(block.customTitle || "Recovery break", block.customDetail)
      : block.source === "CUSTOM" ? { time: "", end: "", title: block.customTitle || "Custom focus block", objective: "Personal focus",
        context: block.customDetail || "Complete the outcome you defined for this block.", status: "Planned", tone: "review" }
      : endOfDay();
    return withTime(formatClockTime(block.startTime), formatClockTime(block.endTime), entry);
  };
  const dayPartMeta: { value: SummaryDayPart; label: string }[] = [
    { value: "MORNING", label: "Morning" }, { value: "AFTERNOON", label: "Afternoon" }, { value: "EVENING", label: "Evening" },
  ];
  const columns = dayPartMeta.map((part) => {
    const blocks = preferences.scheduleBlocks.filter((block) => block.dayPart === part.value)
      .sort((left, right) => left.startTime.localeCompare(right.startTime));
    const entries = blocks.map(resolveBlock);
    return { label: part.label, range: entries.length ? `${entries[0].time}–${entries.at(-1)?.end}` : "", entries };
  ...
  return <section className="panel today-schedule" aria-labelledby="today-schedule-title">
    <header className="today-schedule-head"><div><p className="eyebrow">Morning operating plan</p><h2 id="today-schedule-title">Today’s schedule — {formatScheduleDate(date)}</h2>
      <p>Your locally configured time blocks are populated with the most relevant live work available for each source.</p></div>
      <div className="today-schedule-summary"><span><strong>{columns.length}</strong>dayparts</span><span><strong>{focusBlockCount}</strong>focus blocks</span></div></header>
    <div className={`today-schedule-columns dayparts-${columns.length}`}>{columns.map((column, columnIndex) => <section className="schedule-daypart" key={column.label}>
    ...
    <footer className="today-schedule-foot"><p><strong>Configured locally:</strong> change time blocks, priority sources, specialization, and recommendation emphasis from <b>Customize Summary</b>.</p>
...
function ApplicationFollowUpModal({ item, onClose, onSaved, onError }: {
  ...
  async function dropFollowUp() {
    ...
    try {
      await api(`/api/v1/applications/${item.applicationId}/follow-up`, { method: "PATCH", body: JSON.stringify({
        drop: true, note: "Follow-up removed from Summary by the user.",
      }) });
...
function PersonalSkillBacklogPanel({ overview, busy, onSync, onOpen }: {
  ...
  return <section className="personal-backlog" aria-labelledby="personal-backlog-heading">
    <header className="personal-backlog-heading"><div><p className="section-label">Persisted planning loop</p>
